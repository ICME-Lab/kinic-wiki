...
// Why: UI must name the payment token as KINIC, not the generic "token".
import { kinicBaseUnitsPerToken } from "@/lib/cycles";
...
export function formatTokenAmountFromE8s(value: bigint | string | null | undefined): string {
  ...
  const unitsPerToken = kinicBaseUnitsPerToken();
  const whole = e8s / unitsPerToken;
  const thousandths = (e8s % unitsPerToken) / E8S_PER_DISPLAY_UNIT;
