...
assert.match(profile, /identityLedgerBalanceError: ledgerError/);
assert.match(profile, /Send KINIC/);
assert.match(profile, /Max/);
assert.match(profile, /amountTouched/);
assert.match(profile, /transferableBalanceE8s/);
assert.match(profile, /setTransferStatus\("success"\);[\s\S]*refreshIdentityLedgerBalance\(\)/);
assert.match(profile, /setTransferStatus\("error"\);[\s\S]*return;/);
