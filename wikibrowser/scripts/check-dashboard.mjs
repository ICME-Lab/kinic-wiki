...
assert.match(homePage, /import \{ KINIC_LEDGER_FEE_E8S \} from "@\/lib\/cycles";/);
assert.match(homePage, /const paymentAmountE8s = createDatabasePurchaseAmountE8s\(\);/);
assert.match(homePage, /function createDatabaseRequiredBalanceE8s\(\): bigint/);
assert.match(homePage, /return createDatabasePurchaseAmountE8s\(\) \+ KINIC_LEDGER_FEE_E8S \* 2n;/);
...
assert.match(homePage, /Connect OISY or Plug with at least \$\{formatTokenAmountFromE8s\(createDatabaseRequiredBalanceE8s\(\)\)\} before creating a database\./);
assert.match(homePage, /Create database requires at least \$\{formatTokenAmountFromE8s\(createDatabaseRequiredBalanceE8s\(\)\)\} in the connected wallet\./);
...
assert.match(homePage, /return BigInt\(balanceE8s\) >= createDatabaseRequiredBalanceE8s\(\);/);
...
assert.match(homePage, /requiredBalanceLabel=\{formatTokenAmountFromE8s\(createDatabaseRequiredBalanceE8s\(\)\)\}/);
