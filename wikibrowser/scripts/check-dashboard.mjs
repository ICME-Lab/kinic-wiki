...
const creditsState = readFileSync(new URL("../lib/credits-state.ts", import.meta.url), "utf8");
...
assert.match(createDatabaseDialog, /The database activates after its first purchase\./);
...
assert.match(homeUi, /active && mode === "member" && database\.publicReadable/);
assert.match(homeUi, /active && database\.publicReadable \? <ShareDatabaseLink database=\{database\} \/>/);
assert.match(homeUi, /active \? \(\n\s+<Link className="text-accent no-underline hover:underline" href=\{`\/skills\/\$\{encodeURIComponent\(database\.databaseId\)\}`\}>/);
...
assert.match(wikiBrowser, /databaseCreditsError=\{currentDatabaseCreditReason\}/);
...
assert.match(ingestPanel, /databaseCreditsError/);
assert.match(ingestPanel, /const submitDisabled = busy \|\| !url\.trim\(\) \|\| Boolean\(databaseCreditsError\)/);
...
assert.match(dashboardClient, /<SummaryPanel creditsConfig=\{creditsConfig\} database=\{database\} databaseId=\{databaseId\} principal=\{principal \?\? "anonymous"\} publicReadable=\{database\.publicReadable\} \/>/);
assert.match(dashboardClient, /const isActiveDatabase = database\?\.status === "active";/);
assert.match(dashboardClient, /const canManage = database\?\.role === "owner" && isActiveDatabase && !memberError;/);
assert.match(dashboardClient, /const canDeletePendingDatabase = database\?\.role === "owner" && database\.status === "pending";/);
assert.match(dashboardClient, /databaseId && isActiveDatabase/);
assert.match(dashboardClient, /nextDatabase\?\.role === "owner" && nextDatabase\.status === "active"/);
assert.match(dashboardClient, /nextDatabase\?\.publicReadable && nextDatabase\.status === "active"/);
assert.match(dashboardClient, /canDeletePendingDatabase \? \(/);
assert.match(dashboardClient, /<PendingDatabasePanel busy=\{busy\} busyAction=\{busyAction\} databaseId=\{databaseId\} databaseName=\{database\.name\} onDelete=\{deleteDatabase\} \/>/);
assert.match(dashboardClient, /deleteDatabaseAuthenticated/);
assert.match(dashboardClient, /listDatabaseCreditPendingOperationsAuthenticated/);
assert.match(dashboardClient, /router\.replace\("\/dashboard"\)/);
assert.match(dashboardClient, /setBusyAction\(\{ kind: "delete" \}\)/);
assert.doesNotMatch(dashboardClient, /expectedCreditsBalanceE8s|allowCreditDiscard|allow_balance_writeoff/);
...
assert.match(dashboardUi, /export function PendingDatabasePanel/);
assert.match(dashboardUi, /This database is reserved until the first credit purchase completes/);
assert.match(dashboardUi, /const active = database\?\.status === "active";/);
assert.match(dashboardUi, /const openHref = active && routable/);
...
assert.match(dashboardDangerZone, /Remaining credits will be discarded\./);
assert.match(dashboardDangerZone, /pendingOperationCount/);
assert.match(dashboardDangerZone, /typedDatabaseId === props\.databaseId/);
assert.match(dashboardDangerZone, /const deleteDisabled = props\.busy \|\| hasPendingOperations/);
...
assert.match(dashboardUi, /role or credits state changes/);
assert.match(dashboardUi, /databaseCreditsView/);
assert.match(dashboardUi, /databaseCreditsHref/);
...
assert.match(homeUi, /Credits/);
assert.match(homeUi, /Credits/);
assert.match(homePage, /getCreditsConfig/);
assert.doesNotMatch(creditsState, /Credits unknown/);
assert.doesNotMatch(homeUi, /Credits unknown \/ 0 e8s/);
assert.doesNotMatch(dashboardUi, /Credits unknown \/ 0 e8s/);
