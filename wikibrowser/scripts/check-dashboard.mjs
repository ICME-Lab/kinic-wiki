...
assert.doesNotMatch(vfsIdl, /Deleted: idl\.Null/);
assert.doesNotMatch(vfsClient, /"Hot" in status/);
...
assert.match(wikiBrowser, /void listDatabasesPublic\(canisterId\)[\s\S]*setPublicDatabaseIds\(new Set\(nextPublicDatabases\.map\(\(database\) => database\.databaseId\)\)\);/);
