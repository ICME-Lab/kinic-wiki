...
assert.match(createDatabaseDialog, /The database activates after its first deposit\./);
...
assert.match(dashboardClient, /<SummaryPanel billingConfig=\{billingConfig\} database=\{database\} databaseId=\{databaseId\} principal=\{principal \?\? "anonymous"\} publicReadable=\{database\.publicReadable\} \/>/);
assert.match(dashboardClient, /deleteDatabaseAuthenticated/);
assert.match(dashboardClient, /listDatabaseBillingPendingOperationsAuthenticated/);
assert.match(dashboardClient, /router\.replace\("\/dashboard"\)/);
assert.match(dashboardClient, /setBusyAction\(\{ kind: "delete" \}\)/);
assert.match(dashboardClient, /expectedBillingBalanceE8s: database\.billingBalanceE8s/);
...
assert.match(dashboardDangerZone, /Withdraw balance before deleting\./);
assert.match(dashboardDangerZone, /delete_balance_writeoff|Write off the remaining/);
