...
function parseSourceRunRequest(value: unknown): SourceRunRequest | string {
  ...
  if (!isCanonicalSourcePath(sourcePath)) return "sourcePath must use /Sources/evidence/<provider>/<id>.md";
...
function isCanonicalSourcePath(path: string): boolean {
  const match = path.match(/^\/Sources\/evidence\/[a-z][a-z0-9]{0,31}\/([A-Za-z0-9][A-Za-z0-9._-]{0,127})\.md$/);
