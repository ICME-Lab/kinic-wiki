...
const policySections = Array.from(policyMarkdown.matchAll(/^##\s+(.+)$/gm), (match) => ({
    ...
    <main className="min-h-screen bg-white text-ink">
      ...
      <div className="mx-auto grid max-w-[1080px] gap-10 px-4 py-10 sm:px-6 sm:py-14 lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-16">
        <aside className="lg:sticky lg:top-8 lg:self-start">
          ...
          <nav className="hidden rounded-lg border border-line bg-paper p-4 lg:block" aria-label="Privacy Policy sections">
              ...
              h1({ children }) {
                return (
                  <header className="mb-10 border-b-2 border-accent pb-8">
                    <p className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-accentText">Kinic legal</p>
...
function PolicySectionLinks({ columns = false }: { columns?: boolean }) {
  return (
