  ...
  try {
    ...
    const latest = await processExportTargets(
      ...
      state,
      {
        ...
        async send() {
          return { result: { path: "/Sources/evidence/chatgpt/test.md", created: true, generationQueued: true, generationError: null } };
        }
      },
      2
    );
    ...
    const latest = await processExportTargets(
      ...
      state,
      {
        async send(message) {
          ...
          return { result: { path: "/Sources/evidence/chatgpt/two.md", created: true, generationQueued: true, generationError: null } };
        }
      },
      2
    );
    ...
    const latest = await processExportTargets(
      ...
      state,
      {
        async send() {
          ...
          return { result: { path: "/Sources/evidence/chatgpt/cancelled.md", created: true } };
        }
      },
      1
    );
