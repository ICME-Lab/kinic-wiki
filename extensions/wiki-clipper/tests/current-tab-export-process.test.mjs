  ...
  try {
    ...
    const latest = await processExportTargets(
      ...
      state,
      {
        ...
        async send() {
          return { result: { path: "/Sources/chatgpt/test.md", created: true, generationQueued: true, generationError: null } };
        }
      },
      2
    );
    ...
    const latest = await processExportTargets(
      ...
      state,
      {
        async send(message) {
          ...
          return { result: { path: "/Sources/chatgpt/two.md", created: true, generationQueued: true, generationError: null } };
        }
      },
      2
    );
    ...
    const latest = await processExportTargets(
      ...
      state,
      {
        async send() {
          ...
          return { result: { path: "/Sources/chatgpt/cancelled.md", created: true } };
        }
      },
      1
    );
