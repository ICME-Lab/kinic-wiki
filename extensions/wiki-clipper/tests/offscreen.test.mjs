...
test("searchRecall uses the generator when recallUrl and recallToken are configured", async () => {
  const fetches = [];
  let actorCreated = false;
  setOffscreenDepsForTest({
    fetch: async (url, options) => {
      fetches.push({ url, headers: options.headers, body: JSON.parse(options.body) });
      return new Response(JSON.stringify({
        mode: "reranked",
        results: [
          { path: "/Knowledge/mcp.md", kind: "file", previewExcerpt: "memory about MCP" }
        ]
      }), { status: 200, headers: { "content-type": "application/json" } });
    },
    createVfsActor: async () => {
      actorCreated = true;
      return { async search_nodes() { return { Ok: [] }; } };
    }
  });
  try {
    const result = await searchRecall("MCP agent memory", "https://chatgpt.com/c/current", {
      ...
      recallToken: "recall-secret"
    });
    assert.equal(actorCreated, false, "generator path should not create a canister actor");
    assert.equal(fetches.length, 1);
    assert.equal(fetches[0].url, "https://wiki-generator.kinic.xyz/recall-search");
    assert.equal(fetches[0].headers.authorization, "Bearer recall-secret");
    assert.equal(fetches[0].body.distilledQuery, "memory agent MCP");
    assert.equal(fetches[0].body.databaseId, "team-db");
    assert.equal(fetches[0].body.conversationUrl, "https://chatgpt.com/c/current");
    assert.deepEqual(result.map((entry) => entry.path), ["/Knowledge/mcp.md"]);
    assert.equal(result[0].snippet, "memory about MCP");
    assert.match(result[0].sourceUrl, /\/Knowledge\/mcp\.md$/);
  } finally {
    setOffscreenDepsForTest();
  }
});

test("searchRecall falls back to the canister search when the generator fails", async () => {
  const fetches = [];
  const calls = [];
  setOffscreenDepsForTest({
    fetch: async () => {
      fetches.push("generator");
    ...
    createVfsActor: async () => ({
      async search_nodes(request) {
        calls.push(request);
        return { Ok: [rawRecallHit(request.prefix[0] === "/Knowledge" ? "/Knowledge/mcp.md" : "/Sources/mcp.md", ["content_fts"], -10_000)] };
      }
    })
  });
  try {
    ...
    assert.equal(fetches.length, 1);
    assert.equal(calls.length, 4);
    assert.deepEqual(result.map((entry) => entry.path), ["/Knowledge/mcp.md", "/Sources/mcp.md"]);
  } finally {
    setOffscreenDepsForTest();
  }
});
