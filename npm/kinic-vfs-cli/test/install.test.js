...
test("builds release asset URLs from package version and platform", () => {
  const asset = releaseAsset("0.1.4", "macos-arm64");
  assert.equal(asset.file, "kinic-vfs-cli-v0.1.4-macos-arm64.tar.gz");
  assert.equal(
    asset.tarUrl,
    "https://github.com/ICME-Lab/kinic-wiki/releases/download/v0.1.4/kinic-vfs-cli-v0.1.4-macos-arm64.tar.gz",
  );
  assert.equal(
    asset.shaUrl,
    "https://github.com/ICME-Lab/kinic-wiki/releases/download/v0.1.4/kinic-vfs-cli-v0.1.4-macos-arm64.sha256",
  );
});
...
test("release version script checks Cargo and npm parity", () => {
  ...
  assert.equal(result.stdout.trim(), "v0.1.4");
});
...
test("npm publish guard only allows the matching tag", () => {
  ...
  assert.equal(runPublishGuard({ GITHUB_REF_TYPE: "tag", GITHUB_REF_NAME: "v0.1.4" }).status, 0);
});
