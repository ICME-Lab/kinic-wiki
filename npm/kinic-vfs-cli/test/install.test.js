...
test("builds release asset URLs from package version and platform", () => {
  const asset = releaseAsset("2.0.0", "macos-arm64");
  assert.equal(asset.file, "kinic-vfs-cli-v2.0.0-macos-arm64.tar.gz");
  assert.equal(
    asset.tarUrl,
    "https://github.com/ICME-Lab/kinic-wiki/releases/download/v2.0.0/kinic-vfs-cli-v2.0.0-macos-arm64.tar.gz",
  );
  assert.equal(
    asset.shaUrl,
    "https://github.com/ICME-Lab/kinic-wiki/releases/download/v2.0.0/kinic-vfs-cli-v2.0.0-macos-arm64.sha256",
  );
});
...
test("release version script checks Cargo and npm parity", () => {
  ...
  assert.equal(result.stdout.trim(), "v2.0.0");
});
...
test("npm publish guard only allows the matching tag", () => {
  ...
  assert.equal(runPublishGuard({ GITHUB_REF_TYPE: "tag", GITHUB_REF_NAME: "v2.0.0" }).status, 0);
});
