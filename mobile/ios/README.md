...
The upload script uses `KINIC_IOS_BUILD_NUMBER` when supplied for a production runtime. Otherwise it reads both the latest accepted iOS build and latest upload attempt with the explicitly selected `ASC_PROFILE`, then uses the next unused number. Sandbox mode always performs this lookup so a stale local override or failed upload number cannot be reused. It does not edit the Xcode project.
The upload script automatically loads `mobile/ios/.env.local` and `mobile/ios/.env.testflight.local` when present. Values passed in the invoking shell take precedence over those local defaults. An env-file marketing version that differs from `project.yml` is rejected unless the invoking shell explicitly overrides it, so a release version cannot be silently replaced by a stale local default. Set `ASC_PROFILE` to an authorized `asc` API-key profile stored in macOS Keychain. The script archives and exports locally, verifies Bundle ID/version/build metadata, uploads the IPA with that profile, and requires the matching App Store Connect upload record to reach `COMPLETE`; it does not require exporting the API private key.
