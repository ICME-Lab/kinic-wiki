...
6. If the user explicitly wants durable write-back, hand off to `ingest` instead of growing query-side mutation rules.
...
- Answer-shape rules live in [../wiki-generate/references/query-answer-rules.md](../wiki-generate/references/query-answer-rules.md). Use that file for abstention, extraction, and exact-value behavior.
...
- If the question shape is still unclear after reading `index.md`, follow the current note roles from `WIKI_CANONICALITY.md` rather than inventing ad hoc search order.
