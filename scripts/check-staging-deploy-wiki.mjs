...
const valid = run("https://kinic-wiki-browser-staging.example.workers.dev");
...
for (const invalidOrigin of [
  ...
  const invalid = run(invalidOrigin);
...
function run(origin) {
  const env = { ...process.env, KINIC_VFS_STAGING_II_ORIGIN: origin };
