...
| `config_version` | 課金設定変更の内部管理用 version。公開 purchase request では使わない。 |
...
- 内部 `config_version` は `cycles_per_kinic` または `min_update_cycles` が実際に変わった場合だけ 1 増える。同じ値の更新では増えない。
...
inter-canister call 失敗や response decode 失敗など ledger 結果が曖昧な場合、canister は `ambiguous` pending operation と DB reservation を保持し、cycles ledger と cycles 残高は更新しない。billing authority は ledger block を確認後に `cycles-repair-complete` で反映するか、transfer 未成立を確認して `cycles-repair-cancel` で取り消す。
