...
この文書は現在のコード挙動を正本にした課金仕様である。外部サービス仕様や過去の設計文書ではなく、runtime、canister、wallet UI、既存テストの実装に合わせる。
...
| `config_version` | 課金設定変更の管理用 version。 |
...
- `config_version` は `cycles_per_kinic` または `min_update_cycles` が実際に変わった場合だけ 1 増える。同じ値の更新では増えない。
...
購入実行時の現行 `cycles_per_kinic` で確定する。UI が表示する事前値は見積もりであり、正本は `purchase_database_cycles` の戻り値 `amount_cycles` である。

購入は ledger 転送前に以下を拒否する。
...
ledger が `BadFee` error を返し、`expected_fee` が `u64` に変換できる場合、canister は pending operation 削除を試行し、`icrc2_transfer_from failed: BadFee expected fee ...; re-approve with the current ledger fee and retry` を返す。その他の明示的 ledger error でも pending operation 削除を試行し、caller へ ledger error を返す。この場合 cycles ledger は増えない。

ledger call、response decode、または結果判定が曖昧な場合、canister は `operation_status = "ambiguous"` の pending operation と DB reservation を保持する。cycles ledger は増えない。caller には operation ID を含む repair required error を返す。billing authority は ledger block を確認後、`repair_database_cycles_purchase_complete(database_id, operation_id, ledger_block_index)` で block 内容を pending operation と照合して cycles を反映するか、transfer 未成立を確認して `repair_database_cycles_purchase_cancel(database_id, operation_id)` で取消できる。

ledger 成功後に pending DB activation、migration、または cycles 残高反映が失敗した場合、`completed` pending operation と DB 予約を残す。cycles ledger は増えず、cycles 残高も増えない。caller には operation ID と ledger block index を含む local apply error を返す。billing authority は `retry_database_cycles_purchase(database_id, operation_id)` で再 apply できる。

browser `/cycles` は approve 後の local apply failure を検出すると、payer 向けに database ID、approve block、operation ID、ledger block index、payment amount e8s を表示し、localStorage に保存する。billing authority はその値を受け取り、以下を実行する。

```bash
kinic-vfs-cli --canister-id <canister-id> database cycles-retry <database-id> <operation-id>
kinic-vfs-cli --canister-id <canister-id> database cycles-repair-complete <database-id> <operation-id> <ledger-block-index>
kinic-vfs-cli --canister-id <canister-id> database cycles-repair-cancel <database-id> <operation-id>
```

`cycles-retry` は `completed` operation 専用である。`cycles-repair-complete` と `cycles-repair-cancel` は `ambiguous` operation 専用である。どちらも billing authority identity 以外は拒否される。
...
KINIC 入力は正の数だけ許可する。小数は最大 8 桁、e8s 換算後は `u64::MAX` 以下でなければならない。
...
consent 生成時は request arg を `DatabaseCyclesPurchaseRequest` として decode し、現行 config で cycles を計算する。DB や支払額が購入条件を満たさない場合は unsupported になる。
...
`charge_database_update` は現在残高より請求額が大きい場合、`database cycles balance is insufficient for update charge` を返す。canister wrapper 経由では、更新関数が成功済みで課金に失敗した場合、`cycles charge failed after update: ...` で trap する。
...
残高が不足する場合、支払える分だけ `paid_cycles = max(min(balance_cycles, charge_cycles), 0)` として徴収する。未払い分は繰り越さない。課金後残高が `min_update_cycles` 未満、または `paid_cycles < charge_cycles` の場合は suspended になる。既に suspended の場合は元の `suspended_at_ms` を維持する。
...
Pending cycles operation は公開 API では取得できない。内部では transfer 中の `in_flight` 状態、ledger 成功後 local apply 前の `completed` 状態、memo 照合、同一 caller の重複 purchase guard に使う。
...
inter-canister call 失敗や response decode 失敗など ledger 結果が曖昧な場合も no-credit として cleanup を試行する。実際には transfer が成立していた可能性があっても、自動 repair API と cancel repair API は提供しない。billing authority retry は ledger 成功後に `completed` pending operation が残った場合だけ使う。

ledger transfer 成功後に pending DB activation や cycles apply が失敗した場合は、`completed` pending operation と DB reservation を残す。`database_cycle_ledger` と cycles 残高は反映成功まで更新しない。billing authority retry が成功すると pending operation を削除する。

## 削除

`delete_database(DeleteDatabaseRequest)` は owner のみ実行できる。DB status は `pending` または `active` でなければならない。

削除前に内部 pending cycles operation が 1 件でも存在する場合は拒否する。ledger 成功後の local apply 失敗では `completed` pending operation が残るため、この状態が解消されるまで削除できない。
