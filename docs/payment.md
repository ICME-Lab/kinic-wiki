...
ストレージ課金は `status = active` かつ `active_mount_id IS NOT NULL` の DB だけ対象である。pending、deleted は対象外である。canister timer は 24h ごとに batch settlement を開始する。controller は `settle_database_storage_charges_batch(request)` を手動実行できる。
...
各 active DB は index DB に保存済みの `logical_size_bytes` で課金する。settlement は全 DB を開いて現在 size を再測定しない。`logical_size_bytes` は write/update など既存の size refresh 経路で更新される。`storage_charged_at_ms` が `NULL` の場合は課金せず、課金カーソルを現在時刻に設定する。
