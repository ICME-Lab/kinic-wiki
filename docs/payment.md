...
deploy script は `BILLING_AUTHORITY_ID` が未指定なら `icp identity principal` を `billing_authority_id` に使う。明示的に `BILLING_AUTHORITY_ID` を渡した場合はその値を優先する。`kinic_ledger_canister_id` は支払い ledger canister の ID であり、billing authority でも rate 設定でもない。
...
ledger call、response decode、または結果判定が曖昧な場合、canister は ambiguous no-credit cleanup を実行する。対象 DB が pending の場合は DB 予約ごと破棄し、active の場合は pending operation を削除する。cycles ledger は増えない。caller には `icrc2_transfer_from result ambiguous; cycles were not credited: ...` を返す。
...
- `crates/vfs_runtime/src/lib.rs`: cycles 設定、pending operation、残高反映、billing authority retry、更新課金、ストレージ課金、履歴権限、削除条件。
- `crates/vfs_canister/src/lib.rs`: canister entrypoint、認証、ICRC ledger call、ICRC-21 consent、`retry_database_cycles_purchase`、metered update wrapper。
