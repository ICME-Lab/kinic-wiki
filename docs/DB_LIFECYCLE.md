...
Unit tests do not deploy a ledger. They mock ledger transfer outcomes inside the canister test harness. Mainnet deploys must use `scripts/mainnet/deploy_wiki.sh`. The wrapper defaults to `mainnet-sev`, so fresh installs must set `KINIC_LEDGER_CANISTER_ID` and `BILLING_AUTHORITY_ID` explicitly. For existing production deploys, pass `--environment ic`; the script then resolves unset values from the current canister `get_cycles_billing_config`. The script rejects empty or anonymous values before install. These principal values cannot be changed after init.
