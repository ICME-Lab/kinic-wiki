...
The authorization page offers the normal Internet Identity connection and a production-review-only username/password path. Review credentials authorize a dedicated Ed25519 service identity whose private key and expected principal are stored as Cloudflare Secrets; all MCP tools still call the real canister. The service principal is restricted operationally to `writer` on `openai-review-fixture`. Reviewer sessions use the same PKCE, code, token, scope, expiration, and refresh-replay controls as II sessions. `MCP_REVIEW_ACCESS_VERSION` is checked before authorization-code exchange, refresh rotation, and access-token use, providing explicit credential-session revocation; earlier version values must not be reused.
