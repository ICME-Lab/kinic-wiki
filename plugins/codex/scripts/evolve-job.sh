#!/usr/bin/env bash
# Where: plugins/codex/scripts/evolve-job.sh
# What: Run Codex-driven Kinic skill evolution job prepare/finish steps.
# Why: Codex must own the LLM candidate generation while Kinic CLI owns VFS state.
set -euo pipefail
...
resolve_runtime_path() {
  ...
  printf 'error: Kinic agent runtime not found; reinstall plugin with kinic-vfs-cli codex setup\n' >&2
  exit 69
}
...
case "$command_name" in
  ...
  finish)
    ...
    exec python3 -m kinic_agent_runtime.evolve finish-job "$job_id" --candidate-file "$candidate_file" --cli "$cli" --projection-dir "$projection_dir" --generator codex-plugin --llm-route codex-skill
    ;;
  *)
    usage
    exit 64
