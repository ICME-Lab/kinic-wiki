...
description: Record Kinic Skill Registry run evidence after a Claude Code skill materially affects a task outcome. Use when the user asks to record or preserve skill evidence from Claude Code.
---
...
5. Summarize the script JSON output for the user, including `run_path`.

Do not use MCP for this workflow. The script calls the shared `kinic_agent_runtime.evidence` runner, which runs `kinic-vfs-cli skill record-run --json`.
