...
export async function verifyAppStoreNotification(env, signedPayload) {
...
async function notificationTransactionId(env, data) {
...
async function verifyAppleSignedJws(env, jws) {
    const certificates = certificatesFromHeader(jws);
    validateCertificateChain(env, certificates);
    const signatureOk = verifySignature("sha256", Buffer.from(jwsSigningInput(jws)), { key: certificates[0].publicKey, dsaEncoding: "ieee-p1363" }, Buffer.from(jwsSignature(jws)));
    if (!signatureOk) {
        throw new Error("App Store notification signature is invalid");
    }
    return decodeJwsPayload(jws);
}
function certificatesFromHeader(jws) {
...
function validateCertificateChain(env, certificates) {
...
function notificationData(env, payload) {
...
function validateAppStoreIdentity(env, value, label) {
