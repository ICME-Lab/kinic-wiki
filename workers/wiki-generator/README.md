...
Cloudflare Worker for turning raw sources into review-ready knowledge pages.
...
The fetched raw web evidence written to `/Sources/<provider>/<id>.md` remains a VFS `source` node. Legacy one-segment raw source paths are not accepted by the worker; migrate them explicitly before regeneration or purge operations.
...
2. stores normalized evidence under `/Sources/<provider>/<id>.md`,
3. queues the raw source for knowledge page generation,
