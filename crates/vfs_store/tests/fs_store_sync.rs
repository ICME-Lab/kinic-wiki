    ...
    for page_index in 0..5 {
        let page = store
            .export_snapshot_filtered(
                ...
                |path| path == "/Knowledge/zz-allowed.md",
            )
        ...
        if page.next_cursor.is_none() {
            assert_eq!(allowed_paths, vec!["/Knowledge/zz-allowed.md".to_string()]);
            return;
        }
        cursor = page.next_cursor;
...
#[test]
fn export_snapshot_filtered_ignores_denied_changes_between_pages() {
    let (_dir, store) = new_store();
    let mut first_denied_etag = String::new();
    for index in 0..25 {
        let etag = write_node(
            &store,
            &format!("/Knowledge/aa-denied-{index:03}.md"),
    ...
    let first = store
        ...
        .expect("first filtered snapshot should page");
    assert!(first.nodes.is_empty());
    assert!(first.next_cursor.is_some());

    write_node(
        &store,
        "/Knowledge/aa-denied-000.md",
        "denied update",
        Some(&first_denied_etag),
        200,
    );
    ...
    for _ in 0..5 {
        let page = store
            ...
            .expect("denied-only changes should not invalidate filtered snapshot");
        allowed_paths.extend(page.nodes.into_iter().map(|node| node.path));
        if page.next_cursor.is_none() {
            assert_eq!(allowed_paths, vec!["/Knowledge/zz-allowed.md".to_string()]);
            return;
    ...
    panic!("filtered snapshot should still reach the allowed path");
}
...
fn export_snapshot_filtered_rejects_allowed_update_between_pages() {
    ...
    let allowed_etag = write_node(&store, "/Knowledge/zz-allowed.md", "allowed", None, 100);
...
fn export_snapshot_filtered_rejects_allowed_delete_between_pages() {
    ...
    store
        .delete_node(
            DeleteNodeRequest {
                database_id: "default".to_string(),
                path: "/Knowledge/zz-allowed.md".to_string(),
                expected_etag: Some(allowed_etag),
                expected_folder_index_etag: None,
            },
            200,
        )
...
fn fetch_updates_filtered_advances_across_denied_pages() {
    ...
    for page_index in 0..5 {
        ...
        if page.next_cursor.is_none() {
            assert_eq!(changed_paths, vec!["/Knowledge/zz-allowed.md".to_string()]);
            assert_eq!(removed_paths, vec!["/Knowledge/zz-removed.md".to_string()]);
            return;
        }
        cursor = page.next_cursor;
        target_snapshot_revision = Some(page.snapshot_revision);
