...
fn query_context_filtered_overfetches_after_denied_hits() {
    let (_dir, store) = new_store();
    for index in 0..20 {
        let path = format!("/Knowledge/filter/aa-denied-{index:03}.md");
        ensure_parent_folders(&store, &path, index);
        store
            .append_node(
                AppendNodeRequest {
                    database_id: "default".to_string(),
    ...
    ensure_parent_folders(&store, "/Knowledge/filter/zz-allowed.md", 30);
    store
        .append_node(
            AppendNodeRequest {
                database_id: "default".to_string(),
                path: "/Knowledge/filter/zz-allowed.md".to_string(),
                content: "needle shared context".to_string(),
                expected_etag: None,
                separator: None,
                metadata_json: None,
    ...
    let context = store
        .query_context_filtered(
            ...
            |path| path == "/Knowledge/filter/zz-allowed.md",
        )
