...
fn write_nodes_creates_folder_items() {
    ...
    let results = write_nodes(WriteNodesRequest {
        ...
        nodes: vec![WriteNodeItem {
            path: "/Knowledge/batch-folder".to_string(),
    ...
    .expect("batch folder write should succeed");

    assert_eq!(results.len(), 1);
    assert!(results[0].created);
    assert_eq!(results[0].node.path, "/Knowledge/batch-folder");
    assert_eq!(results[0].node.kind, NodeKind::Folder);
    let children = list_children(ListChildrenRequest {
        database_id: "default".to_string(),
        path: "/Knowledge".to_string(),
    })
    ...
    assert!(children.iter().any(
        |child| child.path == "/Knowledge/batch-folder" && child.kind == NodeEntryKind::Folder
    ));
}
