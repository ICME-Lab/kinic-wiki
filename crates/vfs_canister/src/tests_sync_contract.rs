...
fn production_ii_alternative_origins_do_not_include_localhost() {
    ...
    assert!(!II_PRODUCTION_ALTERNATIVE_ORIGINS_BODY.contains("https://ios-local.kinic.xyz"));
}
...
fn local_dev_ii_alternative_origins_include_fixed_dev_server_origins() {
    ...
    assert!(II_LOCAL_DEV_ALTERNATIVE_ORIGINS_BODY.contains("https://ios-local.kinic.xyz"));
    assert_eq!(
