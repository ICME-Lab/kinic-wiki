...
fn connection_flags_are_optional_in_cli() {
    ...
    let scoped = Cli::try_parse_from(["wiki-cli", "rebuild-scope-index", "--scope", "foo"]);
    assert!(scoped.is_ok(), "scope rebuild command should parse");

    let scoped_path =
        Cli::try_parse_from(["wiki-cli", "rebuild-scope-index", "--scope", "/Wiki/foo"]);
    assert!(scoped_path.is_ok(), "canonical scope path should parse");

    let nested = Cli::try_parse_from(["wiki-cli", "rebuild-scope-index", "--scope", "foo/child"]);
