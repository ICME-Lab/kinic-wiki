...
use vfs_types::{
    ...
    FetchUpdatesRequest, KINIC_LEDGER_FEE_E8S, ListChildrenRequest, ListNodesRequest,
    MarketCreateListingRequest, MarketListing, MarketListingDetail, MarketListingStatus,
    MarketPurchaseRequest, MarketUpdateListingRequest, MkdirNodeRequest, MoveNodeRequest, NodeKind,
    OpsAnswerSessionCheckRequest, OpsAnswerSessionRequest, QueryContextRequest,
    SearchNodePathsRequest, SearchNodesRequest, SearchPreviewMode, SmtPolicyUploadRequest,
...
fn smt_policy_request(smt_lib: &str) -> SmtPolicyUploadRequest {
    SmtPolicyUploadRequest {
        version: "test-v1".to_string(),
        smt_lib: smt_lib.to_string(),
        source_refs: Vec::new(),
        compiler_version: Some("test".to_string()),
        icme_policy_id: None,
        icme_policy_hash: None,
    }
}

fn assert_empty_market_preview(detail: &MarketListingDetail) {
    assert_eq!(detail.verified_stats.total_nodes, 0);
    assert_eq!(detail.verified_stats.wiki_nodes, 0);
    assert_eq!(detail.verified_stats.source_nodes, 0);
    assert_eq!(detail.verified_stats.folder_nodes, 0);
    assert_eq!(detail.verified_stats.markdown_chars, 0);
    assert_eq!(detail.verified_stats.source_chars, 0);
    assert_eq!(detail.verified_stats.link_edges, 0);
    assert_eq!(detail.verified_stats.logical_size_bytes, 0);
    assert_eq!(detail.verified_stats.last_content_updated_at_ms, None);
    assert!(detail.preview.top_level_paths.is_empty());
    assert!(detail.preview.excerpts.is_empty());
    assert!(detail.preview.category_graph.nodes.is_empty());
    assert!(detail.preview.category_graph.edges.is_empty());
    assert!(detail.preview.graph_links.is_empty());
...
fn create_policy_migration_marker(conn: &Connection) {
    conn.execute_batch(
        "CREATE TABLE policy_schema_migrations (
         ...
         VALUES ('policy_db:000_initial', 0);",
    )
    .expect("policy migration marker should create");
}

fn create_operator_policy_table(conn: &Connection) {
    conn.execute_batch(
    ...
    .expect("operator policy table should create");
}

fn create_database_policy_table(conn: &Connection) {
    conn.execute_batch(
    ...
    .expect("database policy table should create");
}

#[test]
fn policy_schema_migrations_are_idempotent() {
    let (service, _) = service_with_root();

    service
        .run_index_migrations()
        .expect("policy migrations should be idempotent");
}

#[test]
fn policy_schema_marker_without_policy_tables_is_rejected() {
    let dir = tempdir().expect("tempdir should create");
    let root = dir.keep();
    let policy_path = root.join("policy.sqlite3");
    let conn = Connection::open(&policy_path).expect("policy db should open");
    create_policy_migration_marker(&conn);
    drop(conn);
    ...
    let error = service
        .run_index_migrations()
        .expect_err("partial policy schema should reject");

    assert!(error.contains("unsupported policy schema"));
    assert!(error.contains("operator_smt_policies"));
}

#[test]
fn policy_schema_missing_database_policy_table_is_rejected() {
    let dir = tempdir().expect("tempdir should create");
    let root = dir.keep();
    let policy_path = root.join("policy.sqlite3");
    let conn = Connection::open(&policy_path).expect("policy db should open");
    create_policy_migration_marker(&conn);
    create_operator_policy_table(&conn);
    drop(conn);
    ...
    assert!(error.contains("unsupported policy schema"));
    assert!(error.contains("database_smt_policies"));
}

#[test]
fn policy_schema_missing_operator_policy_table_is_rejected() {
    let dir = tempdir().expect("tempdir should create");
    let root = dir.keep();
    let policy_path = root.join("policy.sqlite3");
    let conn = Connection::open(&policy_path).expect("policy db should open");
    create_policy_migration_marker(&conn);
    create_database_policy_table(&conn);
    drop(conn);
...
#[test]
fn database_smt_policy_owner_sets_gets_and_clears() {
    let service = service();
    service
        .create_database("policy-db", "owner", 1)
        .expect("database should create");
    service
        .grant_database_access("policy-db", "owner", "writer", DatabaseRole::Writer, 2)
        .expect("writer grant should succeed");

    let smt_lib = r#"
        (declare-const path String)
        (assert (= path "/Wiki/a.md"))
        (check-sat)
    "#;
    let non_owner = service
        .set_database_smt_policy("policy-db", "writer", smt_policy_request(smt_lib), None, 3)
        .expect_err("non-owner should not set database policy");
    assert!(non_owner.contains("database owner or admin required"));

    let set = service
        .set_database_smt_policy("policy-db", "owner", smt_policy_request(smt_lib), None, 4)
        .expect("owner should set database policy");
    service
        .get_database_smt_policy("policy-db", "writer")
        .expect_err("writer should not get database policy");
    let loaded = service
        .get_database_smt_policy("policy-db", "owner")
        .expect("database policy should load")
        .expect("database policy should exist");
    assert_eq!(loaded.smt_lib, smt_lib);
    assert_eq!(loaded.created_by, "owner");

    service
        .clear_database_smt_policy("policy-db", "writer", Some(set.policy.version.clone()))
        .expect_err("non-owner should not clear database policy");
    service
        .clear_database_smt_policy("policy-db", "owner", Some(set.policy.version))
        .expect("owner should clear database policy");
    assert!(
        service
            .get_database_smt_policy("policy-db", "owner")
...
#[test]
fn operator_smt_policy_get_requires_billing_authority() {
    let service = service();
    service
        .get_operator_smt_policy("owner")
        .expect_err("non-admin should not get operator policy");
    service
        .set_operator_smt_policy(
            MARKET_SECOND_BUYER_PRINCIPAL,
            smt_policy_request("(assert true)"),
            None,
            1,
        )
        .expect("operator policy should set");

    let loaded = service
        .get_operator_smt_policy(MARKET_SECOND_BUYER_PRINCIPAL)
        .expect("billing authority should get operator policy")
        .expect("operator policy should exist");

    assert_eq!(loaded.smt_lib, "(assert true)");
}

#[test]
fn operator_smt_policy_deny_filters_database_policy_allow() {
    let service = service();
    service
        .create_database("policy-read-db", "owner", 1)
        .expect("database should create");
    cycle_database(&service, "policy-read-db", "owner", 1_000_000, 1, 2);

    for (path, content, now) in [
        ("/Wiki/public.md", "shared token public", 3),
        ("/Wiki/secret.md", "shared token secret", 4),
    ] {
        service
            .write_node(
                "owner",
                WriteNodeRequest {
                    database_id: "policy-read-db".to_string(),
                    path: path.to_string(),
                    kind: NodeKind::File,
    ...
    service
        ...
        .expect("database allow policy should set");
    service
        .set_operator_smt_policy(
            MARKET_SECOND_BUYER_PRINCIPAL,
            smt_policy_request(
                r#"
                (declare-const path String)
                (assert (not (= path "/Wiki/secret.md")))
                (check-sat)
            "#,
            ),
            None,
        ...
        .expect("operator deny policy should set");

    let denied = service
        .read_node("policy-read-db", "owner", "/Wiki/secret.md")
        .expect_err("operator deny should reject direct read");
    assert!(denied.contains("SMT policy denied read_node"));
    assert_eq!(
        service
            .read_node("policy-read-db", "owner", "/Wiki/public.md")
    ...
    assert!(!hits.iter().any(|hit| hit.path == "/Wiki/secret.md"));
    ...
    assert!(
        context
            .search_hits
            .iter()
            .all(|hit| hit.path != "/Wiki/secret.md")
    );
    assert!(context.nodes.iter().all(|node| {
        node.node.path != "/Wiki/secret.md" && !node.node.content.contains("secret")
    }));
}

#[test]
fn path_allowlist_policy_filters_collection_reads() {
    let service = service();
    service
        .create_database("policy-allowlist-db", "owner", 1)
        .expect("database should create");
    cycle_database(&service, "policy-allowlist-db", "owner", 1_000_000, 1, 2);

    let base_snapshot = service
        .export_fs_snapshot(
            "owner",
            ExportSnapshotRequest {
                database_id: "policy-allowlist-db".to_string(),
                prefix: Some("/Wiki".to_string()),
                limit: 100,
                cursor: None,
    ...
    let mut public_etag = String::new();
    let mut secret_etag = String::new();
    for (path, content, now) in [
        ("/Wiki/aaa-secret.md", "shared token early secret", 3),
        ("/Wiki/public.md", "shared token public", 4),
        ("/Wiki/secret.md", "shared token secret", 5),
    ] {
        let write = service
            .write_node(
                "owner",
            ...
            .expect("node should write");
        match path {
            "/Wiki/public.md" => public_etag = write.node.etag,
            "/Wiki/secret.md" => secret_etag = write.node.etag,
            _ => {}
        }
    }
    ...
    service
        ...
        .expect("path allowlist policy should set");

    assert_eq!(
        service
            .read_node("policy-allowlist-db", "owner", "/Wiki/public.md")
            .expect("public read should succeed")
            .expect("public node should exist")
            .content,
        "shared token public"
    ...
    let denied = service
        .read_node("policy-allowlist-db", "owner", "/Wiki/secret.md")
        .expect_err("secret read should deny");
    assert!(denied.contains("SMT policy denied read_node"));
    ...
    assert!(
        context
            .search_hits
            .iter()
            .all(|hit| hit.path == "/Wiki/public.md")
    );
    assert!(
        context
            .nodes
            .iter()
            .all(|node| node.node.path == "/Wiki/public.md")
    );
    ...
    assert_eq!(snapshot.next_cursor.as_deref(), Some("/Wiki/public.md"));
    ...
    let updates = service
        .fetch_fs_updates(
            ...
            FetchUpdatesRequest {
                database_id: "policy-allowlist-db".to_string(),
                known_snapshot_revision: base_snapshot.snapshot_revision,
                prefix: Some("/Wiki".to_string()),
                limit: 1,
                cursor: None,
    ...
    assert_eq!(updates.next_cursor.as_deref(), Some("/Wiki/public.md"));
    assert!(updates.removed_paths.is_empty());

    let removal_base = snapshot.snapshot_revision;
    service
        .delete_node(
            "owner",
            DeleteNodeRequest {
                ...
                expected_etag: Some(public_etag),
                expected_folder_index_etag: None,
            },
            7,
        )
        .expect("public delete should succeed");
    service
        .delete_node(
            "owner",
            DeleteNodeRequest {
                database_id: "policy-allowlist-db".to_string(),
                path: "/Wiki/secret.md".to_string(),
                expected_etag: Some(secret_etag),
                expected_folder_index_etag: None,
            },
            8,
        )
    ...
    let removals = service
        .fetch_fs_updates(
            ...
            FetchUpdatesRequest {
                database_id: "policy-allowlist-db".to_string(),
                known_snapshot_revision: removal_base,
                prefix: Some("/Wiki".to_string()),
                limit: 100,
                cursor: None,
                target_snapshot_revision: None,
        ...
        .expect("removals should fetch");
    assert!(removals.changed_nodes.is_empty());
    assert_eq!(removals.removed_paths, vec!["/Wiki/public.md"]);
}

#[test]
fn path_allowlist_policy_search_overfetches_to_allowed_hits() {
    let service = service();
    service
        .create_database("policy-search-overfetch-db", "owner", 1)
        .expect("database should create");
    cycle_database(
        &service,
        "policy-search-overfetch-db",
        "owner",
        1_000_000,
        1,
        2,
    ...
    for (path, content, now) in [
        ("/Wiki/a-shared.md", "shared denied content", 3),
        ("/Wiki/z-shared.md", "shared allowed content", 4),
    ] {
        service
            .write_node(
                "owner",
                WriteNodeRequest {
                    database_id: "policy-search-overfetch-db".to_string(),
                    path: path.to_string(),
                    kind: NodeKind::File,
                    content: content.to_string(),
                    metadata_json: "{}".to_string(),
    ...
    assert_eq!(hits[0].path, "/Wiki/z-shared.md");
    assert!(!hits[0].snippet.as_deref().unwrap_or("").contains("denied"));
    assert!(
        !hits[0]
            .preview
            .as_ref()
    ...
    assert_eq!(
        path_hits
            ...
            .collect::<Vec<_>>(),
        vec!["/Wiki/z-shared.md"]
    );
}

#[test]
fn path_allowlist_policy_query_context_overfetches_to_allowed_hits() {
    let service = service();
    service
        .create_database("policy-context-overfetch-db", "owner", 1)
        .expect("database should create");
    cycle_database(
        &service,
        "policy-context-overfetch-db",
        "owner",
        1_000_000,
        1,
        2,
    ...
    for index in 0..12 {
        service
            .write_node(
                "owner",
                WriteNodeRequest {
                    database_id: "policy-context-overfetch-db".to_string(),
                    path: format!("/Wiki/a-denied-{index:02}.md"),
                    kind: NodeKind::File,
                    content: format!("shared context denied {index}"),
                    metadata_json: "{}".to_string(),
                    expected_etag: None,
                },
                3 + index,
    ...
    service
        .write_node(
            ...
            WriteNodeRequest {
                database_id: "policy-context-overfetch-db".to_string(),
                path: "/Wiki/z-allowed.md".to_string(),
                kind: NodeKind::File,
                content: "shared context allowed links to [denied](/Wiki/a-denied-00.md)"
                    .to_string(),
                metadata_json: "{}".to_string(),
                expected_etag: None,
            },
    ...
    assert!(
        context
            .nodes
            .iter()
            .all(|node| !node.node.content.contains("denied "))
    );
    assert!(context.graph_links.iter().all(|link| {
        link.source_path == "/Wiki/z-allowed.md" && link.target_path == "/Wiki/z-allowed.md"
    }));
    assert!(context.evidence.iter().all(|evidence| {
        evidence.node_path == "/Wiki/z-allowed.md"
            && evidence.refs.iter().all(|reference| {
                reference.source_path == "/Wiki/z-allowed.md"
                    && reference.via_path == "/Wiki/z-allowed.md"
            })
    }));
}

#[test]
fn database_sql_json_rejects_when_smt_policy_is_configured() {
    let service = service();
    service
        .create_database("policy-sql-db", "owner", 1)
        .expect("database should create");
    service
        .set_database_smt_policy(
            "policy-sql-db",
        ...
        .expect("database policy should set");
    ...
    let error = service
        ...
        .expect_err("database SQL should reject when policy is configured");
    assert!(error.contains("database SQL is disabled when SMT policy is configured"));
}

#[test]
fn smt_policy_upload_rejects_unsupported_syntax() {
    let service = service();
    service
        .create_database("policy-upload-db", "owner", 1)
        .expect("database should create");
    ...
    let error = service
        .set_database_smt_policy(
            ...
            smt_policy_request(
                r#"
                (declare-const path String)
                (assert (str.contains path "secret"))
                (check-sat)
            "#,
            ),
            None,
        ...
        .expect_err("unsupported SMT fragment should reject");

    assert!(error.contains("unsupported SMT policy") || error.contains("string solver"));
}
...
fn market_listing_detail_returns_verified_preview() {
    ...
    let deny_policy = service
        .set_database_smt_policy(
            ...
            smt_policy_request(
                r#"
                (declare-const path String)
                (assert (not (= path "/Wiki/beta/b.md")))
                (check-sat)
            "#,
            ),
            None,
        ...
        .expect("denylist policy should set");
    let denied_detail = service
        .market_get_listing("2vxsx-fae", &listing.listing_id)
        .expect("public listing metadata should remain readable");
    assert_eq!(denied_detail.listing.listing_id, listing.listing_id);
    assert_empty_market_preview(&denied_detail);
    ...
    service
        .set_database_smt_policy(
            ...
            "seller",
            smt_policy_request("(assert true)"),
            Some(deny_policy.policy.version),
            7,
        )
        .expect("allow policy should replace denylist policy");
    let allow_policy_detail = service
        .market_get_listing("2vxsx-fae", &listing.listing_id)
        .expect("public listing metadata should remain readable");
    assert_eq!(allow_policy_detail.listing.listing_id, listing.listing_id);
    assert_empty_market_preview(&allow_policy_detail);
}
