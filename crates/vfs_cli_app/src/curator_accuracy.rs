...
mod scoring;
use annotations::{
    build_disputes, deterministic_annotation_map, semantic_annotation_key, semantic_annotation_map,
    validate_adjudication, validate_annotation_pair,
};
use scoring::{metrics, record};
...
pub const DETERMINISTIC_RULES: [&str; 16] = crate::curator::rules::DETERMINISTIC_RULES;
const SEMANTIC_KINDS: [&str; 2] = ["contradiction", "duplicate"];

fn deterministic_rule_definitions() -> Vec<DeterministicRuleDefinition> {
    crate::curator::rules::DETERMINISTIC_RULE_DEFINITIONS
        .into_iter()
