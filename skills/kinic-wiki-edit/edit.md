...
2. Use search `--preview-mode content-start` and `query-sql` to reject false positives before editing. Once 2 or more known paths need body checks, use `query-sql` by default.
...
For multi-node planning, `query-sql` is the efficient read path. For mutation safety, `read-node` remains mandatory immediately before each write.
